...
--
-- This file represents the current desired Postgres schema for Fantasy Oscars.
-- The project is pre-launch; we intentionally squash migrations and reset databases
-- (Render + local) instead of preserving historical continuity.
--
-- If you need to regenerate: apply all previous migrations to an empty DB, then
-- pg_dump --schema-only --no-owner --no-privileges and replace this file.
...
--
-- Name: uniq_ceremony_winner_category; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uniq_ceremony_winner_category ON public.ceremony_winner USING btree (category_edition_id);
