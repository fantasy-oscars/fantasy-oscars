...
- Add a test-only path `packages/shared/test/factories/` (not yet created; to be added when migrating).
- Re-export factory builders through `packages/shared/test/factories/index.ts` for easy imports in apps.

Migration steps (not executed yet)
1) Create `packages/shared/test/factories/` and move common builders from `apps/api/test/factories/`.
