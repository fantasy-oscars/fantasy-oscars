...
function DraftCategoryIcon(props: { icon: string; variant: "default" | "inverted" }) {
  if (props.variant === "inverted") {
    return (
      <Box component="span" className="mi-icon mi-icon-tiny dr-icon-punchout" aria-hidden="true">
        {props.icon}
      </Box>
    );
  }
...
export function DraftPlansScreen(props: {
  ...
  if (props.ceremonyState === "loading") return <PageLoader label="Loading draft plans..." />;
            ...
            <Combobox.Dropdown>
              <Combobox.Options>
                ...
                {filteredPlans.length === 0 && planQuery.trim().length === 0 ? (
                  <Combobox.Empty>No plans yet. Type a name to create one.</Combobox.Empty>
                ) : null}
              </Combobox.Options>
            </Combobox.Dropdown>
          </Combobox>
          ...
          <StandardCard component="section" className="nested">
            <Stack gap="xs">
              <Group justify="space-between" wrap="nowrap">
                ...
                {o.saving ? (
                  <Text className="baseline-textMeta">Saving…</Text>
                ) : null}
              </Group>
